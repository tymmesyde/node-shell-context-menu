...
tape('registerCommand with one arg', async t => {
	try {
	...
	t.equal(await Registry.get(`HKCU\\Software\\Classes\\*\\shell\\${options.name}\\command`, ''), '"C:\\Windows\\explorer.exe" "/e" "%1"');
	t.end();
});

tape('registerDirectoryCommand with one arg', async t => {
	try {
	...
	t.equal(await Registry.get(`HKCU\\Software\\Classes\\Directory\\shell\\${options.name}\\command`, ''), '"C:\\Windows\\explorer.exe" "/e" "%1"');
	t.end();
});

tape('removeCommand with one arg', async t => {
...
tape('removeDirectoryCommand with one arg', async t => {
